...
.tw{border:1px solid var(--bdr);border-radius:8px;overflow-x:auto}table{width:100%;border-collapse:collapse;font-size:12px}
...
{`{
  ...
  {tab==="signal_review"&&!loading&&!clientMode&&(()=>{
    ...
    const STA={qualified:{bg:"var(--grn-d)",fg:"var(--grn)",lbl:"Qualified"},unqualified:{bg:"rgba(245,158,11,.15)",fg:"var(--amb)",lbl:"Unqualified"},duplicate:{bg:"var(--hover)",fg:"var(--t2)",lbl:"Duplicate"},demoted:{bg:"rgba(224,108,117,.15)",fg:"var(--red)",lbl:"Demoted"}};
    ...
    // Promote: archive row (unqualified OR demoted) → live Task. Optional feedback
    // teaches the scorer "this SHOULD have qualified" — fed into the rule's
    // Reviewer Feedback memory, which every future relevance check reads.
    const promote=async r=>{const fb=window.prompt(`Promote this ${r.status} result (score ${r.score}) to a live Task?\n\n${r.signal}\n\nOptional: tell the scorer WHY this should have qualified — future scans learn from it. Leave blank to just promote.`,"");if(fb===null)return;const nt={Company:r.company,"Task Rule":r.rule,Score:r.score,"Score Reason":r.reason,"Scan Target":r.target,Signal:r.signal,Source:r.source,URL:r.url,"Task Type":r.type,Date:r.date,Created:new Date().toISOString()};if(r.name)nt.Name=r.name;if(r.leadTitle)nt["Lead Title"]=r.leadTitle;if(r.linkedinUrl)nt["LinkedIn URL"]=r.linkedinUrl;try{const res=await at("create","Tasks",{records:[nt]},bid);setTasks(p=>[...(res.records||[]),...p]);await at("delete","Signal Archive",{recordIds:[r.id]},bid);setArchiveRecs(p=>(p||[]).filter(x=>x.id!==r.id));}catch(e){alert("Promote failed: "+e.message);return;}
      if(fb.trim()){try{const fr=await fetch("/api/review-feedback",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"promote_feedback",baseId:bid,campaignAirtableId:camp?.airtableId,feedback:fb.trim(),task:{company:r.company,rule:r.rule,type:r.type,score:r.score,signal:r.signal}})});const fd=await fr.json().catch(()=>({}));if(!fr.ok||fd.feedbackSaved===false)alert("Promoted, but feedback was not saved: "+(fd.feedbackError||fd.error||("HTTP "+fr.status)));}catch(e){alert("Promoted, but feedback was not saved: "+e.message);}}};
    // Demote: qualified Task → Signal Archive (status "demoted") with REQUIRED
    // feedback. The server moves the record and appends the lesson to the rule's
    // Reviewer Feedback (or the campaign's LinkedIn Posts Feedback for
    // linkedin_engagement) so the next scan doesn't repeat the mistake.
    const demote=async r=>{const fb=window.prompt(`Demote this qualified task (score ${r.score})?\n\n${r.signal}\n\nREQUIRED: tell the scorer WHY this is not a real signal — future relevance checks learn from it.`,"");if(fb===null)return;if(!fb.trim()){alert("Feedback is required — it's what teaches the scorer not to repeat this.");return;}
      try{const res=await fetch("/api/review-feedback",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"demote",baseId:bid,campaignAirtableId:camp?.airtableId,feedback:fb.trim(),task:{id:r.id,company:r.company,rule:r.rule,type:r.type,score:r.score,reason:r.reason,signal:r.signal,source:r.source,url:r.url,scanTarget:r.target,leadTitle:r.leadTitle,name:r.name,linkedinUrl:r.linkedinUrl,date:r.date}})});const d=await res.json().catch(()=>({}));
        if(!res.ok||d.error){alert("Demote failed: "+(d.error||("HTTP "+res.status)));return;}
        setTasks(p=>p.filter(t=>t.id!==r.id));
        if(d.archived?.length)setArchiveRecs(p=>[...d.archived,...(p||[])]);
        const warn=[];if(d.taskDeleted===false)warn.push(d.deleteError);if(d.feedbackSaved===false)warn.push("Feedback memory write failed: "+(d.feedbackError||""));
        if(warn.length)alert("Demoted with warnings:\n"+warn.join("\n"));
    ...
    return(<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:14}}>
        {[{k:"Accounts in view",v:[...new Set(scoped.map(r=>r.company).filter(Boolean))].length},{k:"Rules firing",v:[...new Set(scoped.map(r=>r.rule).filter(Boolean))].length},{k:"Qualified",v:cnt("qualified"),c:"var(--grn)"},{k:"Unqualified",v:cnt("unqualified"),c:"var(--amb)"},{k:"Duplicates",v:cnt("duplicate"),c:"var(--t2)"},{k:"Demoted",v:cnt("demoted"),c:cnt("demoted")?"var(--red)":"var(--t2)"},{k:"Stale / changed roles",v:staleCount,c:staleCount?"var(--red)":"var(--t2)"}].map(s=>(
      ...
      {/* Status pills */}
      <div className="fb">
        {[{v:"all",l:"All"},{v:"qualified",l:"Qualified"},{v:"unqualified",l:"Unqualified"},{v:"duplicate",l:"Duplicates"},{v:"demoted",l:"Demoted"}].map(p=>(
      ...
      <div className="tw"><table><thead><tr>
        ...
        <td style={{whiteSpace:"nowrap"}}>{(r.status==="unqualified"||r.status==="demoted")&&<button className="btn btn-s" style={{fontSize:9,padding:"3px 7px",color:"var(--grn)",borderColor:"rgba(93,168,122,.3)"}} title="Promote to a live Task (optional feedback teaches the scorer)" onClick={()=>promote(r)}>▲ Promote</button>}{r.status==="qualified"&&<button className="btn btn-s" style={{fontSize:9,padding:"3px 7px",color:"var(--red)",borderColor:"rgba(224,108,117,.3)"}} title="Demote — not a real signal. Your feedback is fed into every future relevance check for this rule." onClick={()=>demote(r)}>▼ Demote</button>}{r.status!=="qualified"&&<button className="btn btn-d btn-s" style={{marginLeft:4}} title="Delete from archive" onClick={()=>del("Signal Archive",[r.id],setArchiveRecs)}><I.Trash/></button>}</td>
