...
export default function SignalScope() {
  ...
  // ─── Save rule (any type — signal or top_x) ─────────────────
  const saveRule = async (rule) => {
    let fields;
    if (rule.taskType === "top_x") {
      // Top X rule
      fields = { Name: rule.name, Description: rule.description || "", "Task Type": "top_x", "Scan Target": rule.scanTarget || "leads", "Top N": rule.topN || 10, "Scoring Fields": JSON.stringify(rule.scoringFields || []), Ease: rule.ease || "Medium", Strength: rule.strength || "Strong" };
      try {
  ...
  {showExportModal&&<ExportModal tasks={selCount>0?fTasks.filter(t=>selectedTasks.has(t.id)):fTasks} onClose={()=>setShowExportModal(false)}/>}
...
function RuleEditor({rule,onSave,onClose,availableFields}){
  ...
  const [mode, setMode] = useState(isTopX ? "top_x" : "signal"); // signal | top_x
  ...
  return(<div className="modal-o" onClick={e=>e.target===e.currentTarget&&onClose()}><div className="modal" style={{maxWidth:mode==="top_x"?620:560}}><div className="modal-h"><span style={{fontWeight:600}}>{f.airtableId?"Edit Rule":"New Rule"}</span><button className="btn btn-s" onClick={onClose}>✕</button></div>
  <div className="modal-b">
  ...
  <div className="ig">
    ...
    <div style={{display:"flex",gap:8}}>
      <button className={"btn btn-s"+(mode==="signal"?" btn-p":"")} onClick={()=>setMode("signal")} style={{flex:1,justifyContent:"center"}}>📰 Signal (News / Jobs)</button>
      <button className={"btn btn-s"+(mode==="top_x"?" btn-p":"")} onClick={()=>setMode("top_x")} style={{flex:1,justifyContent:"center"}}>🎯 Top X Scoring</button>
    </div>
  </div>

  {/* Shared fields */}
  <div className="ig"><div className="il">Name</div><input className="inp" value={f.name} onChange={e=>sF(p=>({...p,name:e.target.value}))} placeholder={mode==="top_x"?"e.g. Top 50 most engaged leads":"e.g. CMO / CGO opening"}/></div>
...
function ExportModal({ tasks, onClose }) {
  const allCols = [...new Set(tasks.flatMap(t => Object.keys(t.fields || {})))];
  const defaultCols = ["Company","Task Rule","Score","Scan Target","Signal","Source","Task Type","Date","URL"];
  const [selectedCols, setSelectedCols] = useState(() => allCols.filter(c => defaultCols.includes(c)));
  ...
  const doExport = () => {
    if (!filteredTasks.length || !selectedCols.length) return;
    const csvRows = [selectedCols.map(c => '"' + c.replace(/"/g, '""') + '"').join(",")];
    filteredTasks.forEach(t => {
      const f = t.fields || {};
      csvRows.push(selectedCols.map(c => '"' + String(f[c] || "").replace(/"/g, '""') + '"').join(","));
    });
    const blob = new Blob([csvRows.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "signalscope-tasks-" + new Date().toISOString().slice(0, 10) + ".csv";
    a.click();
    URL.revokeObjectURL(url);
    onClose();
  };

  return (<div className="modal-o" onClick={e => e.target === e.currentTarget && onClose()}><div className="modal" style={{ maxWidth: 640 }}>
    ...
    <div className="modal-b">
      ...
      <div className="ig">
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {taskTypes.length === 0 ? <span style={{ fontSize: 11, color: "var(--t3)" }}>No tasks to export</span> :
            taskTypes.map(t => (
              <button key={t} className={"stag" + (exportTypes.has(t) ? " sel" : "")} onClick={() => toggleType(t)}>
                {t.replace(/_/g, " ")}
              </button>
            ))
          }
      ...
      {/* Columns */}
      <div className="ig">
        <div className="il">Columns to Export</div>
        <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
          <button className="btn btn-s" style={{ fontSize: 9, padding: "2px 6px" }} onClick={() => setSelectedCols([...allCols])}>Select All</button>
          <button className="btn btn-s" style={{ fontSize: 9, padding: "2px 6px" }} onClick={() => setSelectedCols([])}>Clear</button>
          <button className="btn btn-s" style={{ fontSize: 9, padding: "2px 6px" }} onClick={() => setSelectedCols(allCols.filter(c => defaultCols.includes(c)))}>Defaults</button>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {allCols.map(c => (
            <label key={c} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 4, border: "1px solid " + (selectedCols.includes(c) ? "var(--acc)" : "var(--bdr)"), background: selectedCols.includes(c) ? "var(--acc-d)" : "var(--card)", cursor: "pointer", fontSize: 11, color: selectedCols.includes(c) ? "var(--acc)" : "var(--t2)", transition: "all .15s" }}>
              <input type="checkbox" checked={selectedCols.includes(c)} onChange={() => toggleCol(c)} style={{ accentColor: "var(--acc)", width: 12, height: 12 }} />
              {c}
            </label>
          ))}
        </div>
      ...
      <div style={{ padding: 12, background: "var(--hover)", borderRadius: 8, fontSize: 11 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ color: "var(--t2)" }}>
            <strong style={{ color: "var(--t1)" }}>{filteredTasks.length}</strong> tasks · <strong style={{ color: "var(--t1)" }}>{selectedCols.length}</strong> columns
          </span>
          {filteredTasks.length > 0 && selectedCols.length > 0 && (
            <span style={{ fontSize: 10, color: "var(--grn)" }}>Ready to export</span>
          )}
        </div>
        {exportTypes.size > 0 && exportTypes.size < taskTypes.length && (
          <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 4 }}>
            Types: {[...exportTypes].map(t => t.replace(/_/g, " ")).join(", ")}
          </div>
        )}
        {(exportFrom || exportTo) && (
          <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 2 }}>
            Date: {exportFrom || "…"} → {exportTo || "…"}
          </div>
        )}
      </div>
    ...
    <div className="modal-f">
      <button className="btn" onClick={onClose}>Cancel</button>
      <button className="btn btn-p" disabled={!filteredTasks.length || !selectedCols.length} onClick={doExport}>
