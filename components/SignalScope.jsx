  ...
  {tab==="leads"&&!loading&&(()=>{
    ...
    return (<div><div className="ph"><div><div className="pt">Leads</div><div className="pd">{filteredLeads.length}{campTagFilter!=="all"||engagementFilter!=="all"?` of ${leads.length}`:""} contacts{engagedCount>0?` · ${engagedCount} engaged this week`:""}</div></div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        {!clientMode&&<button className="btn btn-s btn-p" onClick={()=>{console.log("[Movement Scan] button clicked, opening modal");setShowLeadMovementModal(true);}} title="Scan LinkedIn for newly hired / promoted / exited leads">🧭 Movement Scan</button>}
      </div></div>
