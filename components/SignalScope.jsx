...
export default function SignalScope() {
  ...
  const [csvModal, setCsvModal] = useState(null); // {table, setter, headers, rows, mappings}
  const [setupStatus, setSetupStatus] = useState(null); // null | "loading" | {created, skipped, errors}

  useEffect(() => { if (campaign) loadAll(); }, [campaign]);

  const setupAirtable = async () => {
    setSetupStatus("loading");
    try {
      const result = await at("setup", "", {});
      setSetupStatus(result);
      console.log("[Airtable Setup]", result);
    } catch (e) {
      setSetupStatus({ created: [], skipped: [], errors: [e.message] });
    }
  };
  ...
  const FIELD_ALIASES = {
    Accounts: {
      Name: ["name","company","company name","account","account name","accounts","organization","org","org name","business","business name"],
      Domain: ["domain","website","company website","web","url","company url","company domain","site"],
      Industry: ["industry","vertical","sector","category","business type"],
      Size: ["size","employees","employee count","company size","headcount","num employees","employee range","# employees","number of employees","team size"],
      "LinkedIn URL": ["linkedin","linkedin url","linkedin company","company linkedin","linkedin link","li url","linkedin profile"],
      Country: ["country","location","hq","headquarters","hq location","region","geo","geography","city","state"],
    },
    Leads: {
      Name: ["name","full name","contact name","contact","person","lead name","lead"],
      Email: ["email","email address","work email","business email","e-mail","mail","contact email"],
      Title: ["title","job title","position","role","designation","job role","current title"],
      Company: ["company","organization","employer","company name","org","account","account name"],
      "LinkedIn URL": ["linkedin","linkedin url","linkedin profile","profile url","li url","linkedin link","profile"],
      Phone: ["phone","phone number","direct phone","mobile","cell","telephone","contact phone","work phone"],
    },
  };

  const autoDetectMapping = (csvHeaders, table) => {
    ...
    for (const header of csvHeaders) {
      ...
      if (!matched) mappings[header] = header; // keep as-is
    }
    return mappings;
  };

  // ─── Step 1: Parse CSV and open mapping modal ──────────────
  const handleCSVFile = (file, table, setter) => {
    ...
    reader.onload = (e) => {
      ...
      const mappings = autoDetectMapping(headers, table);
      setCsvModal({ table, setter, headers, rows, mappings });
    };
    reader.readAsText(file);
  };

  // ─── Step 2: Upload mapped data ────────────────────────────
  const uploadMappedCSV = async () => {
    if (!csvModal) return;
    const { table, setter, headers, rows, mappings } = csvModal;
    // Filter out skipped columns
    const activeMappings = Object.entries(mappings).filter(([_, v]) => v !== "__skip__");
    const recs = rows.map(row => {
      const obj = {};
      activeMappings.forEach(([csvCol, airtableField]) => {
        const idx = headers.indexOf(csvCol);
        if (idx >= 0 && row[idx]) obj[airtableField] = row[idx];
      });
      return obj;
    ...
    try {
      setLoading(true);
      setCsvModal(null);
      const res = await at("create", table, { records: recs });
  ...
  const exportTasksCSV = () => {
    const data = (filter.src !== "all" || filter.target !== "all" || filter.q || filter.from || filter.to) ? fTasks : tasks;
    if (!data.length) return;
    const allFields = new Set();
    data.forEach(t => Object.keys(t.fields || {}).forEach(k => allFields.add(k)));
    const cols = Array.from(allFields);
    const csvRows = [cols.map(c => '"' + c.replace(/"/g, '""') + '"').join(",")];
    data.forEach(t => {
      const f = t.fields || {};
      csvRows.push(cols.map(c => '"' + String(f[c] || "").replace(/"/g, '""') + '"').join(","));
    });
    const blob = new Blob([csvRows.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "signalscope-tasks-" + new Date().toISOString().slice(0, 10) + ".csv";
  ...
  <div style={{padding:"12px 16px",borderTop:"1px solid var(--bdr)"}}>
    <div style={{display:"flex",gap:6,marginBottom:6}}>
      <button className="btn btn-s" style={{flex:1,justifyContent:"center",fontSize:10}} onClick={async()=>{
        setSetupStatus("testing");
        try{const r=await at("test","",{});setSetupStatus({test:r})}catch(e){setSetupStatus({test:{steps:[{step:"API Call",ok:false,msg:e.message}]}})}
      }} disabled={setupStatus==="loading"||setupStatus==="testing"}>
        {setupStatus==="testing"?"⏳ Testing...":"🧪 Test"}
      </button>
      <button className="btn btn-s btn-p" style={{flex:1,justifyContent:"center",fontSize:10}} onClick={setupAirtable} disabled={setupStatus==="loading"||setupStatus==="testing"}>
        {setupStatus==="loading"?"⏳ Setting up...":"🔧 Setup"}
      </button>
    </div>
    {setupStatus&&setupStatus!=="loading"&&setupStatus!=="testing"&&(<div style={{marginTop:4,fontSize:9,lineHeight:1.6,maxHeight:150,overflowY:"auto"}}>
      {/* Test results */}
      {setupStatus.test&&setupStatus.test.steps?.map((s,i)=>(<div key={i} style={{color:s.ok?"var(--grn)":"var(--red)",marginBottom:2}}>{s.ok?"✅":"❌"} <strong>{s.step}:</strong> {s.msg}</div>))}
      {/* Setup results */}
      {setupStatus.created?.length>0&&<div style={{color:"var(--grn)"}}>✅ Created {setupStatus.created.length} fields: {setupStatus.created.slice(0,5).join(", ")}{setupStatus.created.length>5?"...":""}</div>}
      {setupStatus.skipped?.length>0&&<div style={{color:"var(--t3)"}}>⏭ {setupStatus.skipped.length} fields already exist</div>}
      {setupStatus.tables_found&&<div style={{color:"var(--t3)"}}>📋 Tables: {setupStatus.tables_found.join(", ")}</div>}
      {setupStatus.errors?.length>0&&setupStatus.errors.map((e,i)=><div key={i} style={{color:"var(--red)"}}>❌ {e}</div>)}
      {setupStatus.created&&setupStatus.created.length===0&&setupStatus.errors?.length===0&&!setupStatus.test&&<div style={{color:"var(--grn)"}}>✅ All columns exist!</div>}
    </div>)}
  </div>
  </div>
  ...
  {/* TASKS */}
  {tab==="tasks"&&!loading&&(<div><div className="ph"><div><div className="pt">Tasks</div><div className="pd">{fTasks.length} signal tasks</div></div><div style={{display:"flex",gap:8}}><button className="btn btn-s" onClick={exportTasksCSV} disabled={!tasks.length}><I.Download/> Export CSV</button><button className="btn btn-p btn-s" onClick={startScan} disabled={scanning||!accounts.length||!rules.length}>{scanning?"Scanning... "+Math.round(scanProg)+"%":<><I.Play/> Run Scan</>}</button></div></div>
  ...
  <div className="modal-h"><span style={{fontWeight:600}}>Map CSV Columns → {csvModal.table}</span><button className="btn btn-s" onClick={()=>setCsvModal(null)}>✕</button></div>
  <div className="modal-b">
  <div style={{fontSize:11,color:"var(--t3)",marginBottom:12}}>{csvModal.rows.length} rows found · Map each CSV column to a field or skip it</div>
  <div style={{display:"flex",flexDirection:"column",gap:8}}>
  {csvModal.headers.map((h,i)=>{
    const mapping = csvModal.mappings[h] || "__skip__";
    const targetFields = csvModal.table === "Accounts" ? ["Name","Domain","Industry","Size","LinkedIn URL","Country"] : ["Name","Email","Title","Company","LinkedIn URL","Phone"];
    const isAutoDetected = Object.values(FIELD_ALIASES[csvModal.table]||{}).some(alts => alts.includes(h.toLowerCase().trim()));
    const sample = csvModal.rows.slice(0,3).map(r=>r[i]).filter(Boolean).join(", ");
    return (<div key={h} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",border:"1px solid var(--bdr)",borderRadius:6,background:mapping==="__skip__"?"transparent":"var(--card)"}}>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:12,fontWeight:500,color:mapping==="__skip__"?"var(--t3)":"var(--t1)"}}>{h}</div>
        <div style={{fontSize:9,color:"var(--t3)",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sample||"(empty)"}</div>
      </div>
      <span style={{fontSize:10,color:"var(--t3)"}}>→</span>
      <select className="inp" style={{width:160,padding:"5px 8px",fontSize:11}} value={mapping} onChange={e=>{
        setCsvModal(prev=>({...prev,mappings:{...prev.mappings,[h]:e.target.value}}));
      }}>
        <option value="__skip__">⊘ Skip this column</option>
        {targetFields.map(f=>(<option key={f} value={f}>{f}</option>))}
        <option value={h}>Keep as "{h}"</option>
      </select>
      {isAutoDetected&&mapping!=="__skip__"&&<span style={{fontSize:8,color:"var(--grn)",fontWeight:600}}>AUTO</span>}
    </div>);
  })}
  </div>
  <div style={{marginTop:12,padding:10,background:"var(--hover)",borderRadius:6,fontSize:10,color:"var(--t3)"}}>
    <strong style={{color:"var(--t2)"}}>Preview:</strong> {csvModal.rows.length} rows will be imported with {Object.values(csvModal.mappings).filter(v=>v!=="__skip__").length} columns mapped
  </div>
  </div>
  <div className="modal-f">
    <button className="btn" onClick={()=>setCsvModal(null)}>Cancel</button>
    <button className="btn btn-p" onClick={uploadMappedCSV} disabled={Object.values(csvModal.mappings).filter(v=>v!=="__skip__").length===0}>
      <I.Upload/> Import {csvModal.rows.length} rows
    </button>
  </div>
  </div></div>)}
  </>);
