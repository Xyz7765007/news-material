...
export default function SignalScope() {
  ...
  const saveCampaign = async (data) => {
    const res = await at("create_campaign", "", { fields: { Name: data.name, "Base ID": data.baseId, Features: data.features.join(","), Status: "Active", Description: data.desc || "", Emoji: data.emoji || "📊", Tables: data.tables || "" } });
  ...
  const updateCampaignBase = async (newBaseUrl) => {
    ...
    if (camp.airtableId) {
      ...
      const res = await at("create_campaign", "", { fields: { Name: camp.name, "Base ID": newBaseId, Features: (camp.features || []).join(","), Status: "Active", Description: camp.desc || "", Emoji: camp.emoji || "📊", Tables: info.tableNames.join(", ") } });
      const r = (res.records || [])[0];
      if (r) {
  ...
  <div style={{padding:"12px 16px",borderTop:"1px solid var(--bdr)"}}>
    ...
    <div style={{marginBottom:10}}>
      ...
      {editingBase ? (
        <div style={{display:"flex",flexDirection:"column",gap:4}}>
          <input className="inp" style={{fontSize:10,padding:"5px 8px"}} value={baseInput} onChange={e=>setBaseInput(e.target.value)} placeholder="Paste base URL or ID…" autoFocus onKeyDown={e=>{if(e.key==="Escape"){setEditingBase(false);setBaseInput("")}}}/>
          <div style={{display:"flex",gap:4}}>
            <button className="btn btn-s btn-p" style={{flex:1,justifyContent:"center",fontSize:9}} disabled={!baseInput.trim()} onClick={async()=>{
              try{await updateCampaignBase(baseInput.trim());setEditingBase(false);setBaseInput("")}catch(e){alert("Failed: "+e.message)}
            }}>Connect</button>
            <button className="btn btn-s" style={{fontSize:9}} onClick={()=>{setEditingBase(false);setBaseInput("")}}>Cancel</button>
          </div>
        </div>
      ) : (
    ...
    {setupStatus&&setupStatus!=="loading"&&setupStatus!=="testing"&&(<div style={{marginTop:4,fontSize:9,lineHeight:1.6,maxHeight:120,overflowY:"auto"}}>
      ...
      {setupStatus.created?.length>0&&<div style={{color:"var(--grn)"}}>✅ Created {setupStatus.created.length} fields</div>}
      {setupStatus.skipped?.length>0&&<div style={{color:"var(--t3)"}}>⏭ {setupStatus.skipped.length} exist</div>}
      ...
      {setupStatus.created?.length===0&&!setupStatus.errors?.length&&!setupStatus.test&&<div style={{color:"var(--grn)"}}>✅ All good!</div>}
    </div>)}
  </div>
  </div>
