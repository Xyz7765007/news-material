    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {/* ── SCORING PROMPT — Transparent AI Qualification Layer ── */}
          <div style={{ marginTop: 12, padding: 14, border: "1px solid rgba(191,163,90,0.3)", borderRadius: 8, background: "rgba(191,163,90,0.05)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: 14 }}>🎯</span>
              <span style={{ fontSize: 12, fontWeight: 600, color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Scoring Prompt</span>
              <span style={{ fontSize: 10, color: "var(--text-tertiary)", fontWeight: 400 }}>— AI uses this to score each signal's relevance (0-100)</span>
            </div>
            <textarea className="input textarea" placeholder="Describe how the AI should evaluate and score signals for this task. Auto-generated if left empty."
              value={form.scoringPrompt || ""} onChange={e => setForm(f => ({ ...f, scoringPrompt: e.target.value }))}
              style={{ minHeight: 90, fontSize: 11, lineHeight: 1.6, background: "var(--bg-card)", border: "1px solid var(--border)" }} />
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
              <button className="btn btn-ai" style={{ fontSize: 10, padding: "4px 12px" }} disabled={aiLoading || !form.name}
                onClick={async () => {
                  setAiLoading(true);
                  try {
                    const res = await fetch("/api/classify", {
                      ...
                      body: JSON.stringify({
                        ...
                        taskJobTitleKeywords: form.jobTitleKeywords,
              ...
              <span style={{ fontSize: 9, color: "var(--text-tertiary)" }}>
                {form.scoringPrompt ? `${form.scoringPrompt.length} chars` : "Will auto-generate on save"}
              </span>
            </div>
            <div style={{ marginTop: 8, fontSize: 10, color: "var(--text-tertiary)", lineHeight: 1.5 }}>
              Scoring guide: <span style={{ color: "var(--green)" }}>90-100 = exact match</span> · <span style={{ color: "var(--accent)" }}>70-89 = strong</span> · <span style={{ color: "var(--text-secondary)" }}>50-69 = partial</span> · <span style={{ color: "var(--red)" }}>below 50 = weak</span>
            </div>
          </div>
        </div>
          ...
          <button className="btn btn-primary" disabled={!form.name.trim()} onClick={async () => {
            ...
            if (!scoringPrompt && form.name.trim()) {
              try {
                const res = await fetch("/api/classify", {
                  method: "POST", headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ action: "generate_scoring_prompt", taskName: form.name, taskDescription: form.description, taskKeywords: form.keywords, taskJobTitleKeywords: form.jobTitleKeywords, taskSources: form.sources }),
                });
...
export default function SignalScope() {
  const [appStep, setAppStep] = useState(0); // 0: upload, 1: task defs, 2: review prompts, 3: scoring threshold, 4: dashboard
  ...
  const handleTaskDefsComplete = () => {
    setAppStep(2); // Go to prompt review
  };
  const handlePromptsComplete = () => {
    setAppStep(3); // Go to scoring threshold
  };
  const handleScoringComplete = () => {
    setAppStep(4); // Go to dashboard + auto-scan
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          <StepIndicator current={appStep} steps={["Upload Companies", "Define Tasks", "Review Prompts", "Set Threshold", "Track Signals"]} onNavigate={navigateTo} />
          ...
          {appStep === 3 && (
            <div className="fade-up">
              <div className="section"><h2 className="section-title">Set Relevance Threshold</h2><p className="section-desc">Set the minimum AI relevance score (0-100) for a signal to become a task.</p></div>
          ...
          {/* ─── STEP 2: Review Scoring Prompts ─── */}
          {appStep === 2 && (
            <div className="fade-up">
              <div className="card">
                <div className="card-header">
                  <div>
                    <div className="card-title" style={{ display: "flex", alignItems: "center", gap: 8 }}>🎯 Review Scoring Prompts</div>
                    <div style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 4 }}>
                      These prompts tell the AI how to score each signal's relevance (0-100). Edit any prompt before scanning to fine-tune what gets through.
                    </div>
                  </div>
                  <button className="btn btn-primary btn-sm" onClick={handlePromptsComplete}><Icons.ArrowRight /> Set Threshold</button>
                </div>
                <div className="card-body" style={{ maxHeight: "65vh", overflowY: "auto" }}>
                  {taskDefs.map((task, idx) => (
                    <div key={task.id} style={{ marginBottom: 16, padding: 14, border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-card)" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 4,
                            background: task.signalSource === "job_post" ? "var(--blue-dim)" : task.signalSource === "both" ? "rgba(191,163,90,0.15)" : "var(--green-dim)",
                            color: task.signalSource === "job_post" ? "var(--blue)" : task.signalSource === "both" ? "var(--accent)" : "var(--green)" }}>
                            {task.signalSource === "job_post" ? "📋 JOB" : task.signalSource === "both" ? "📰📋 BOTH" : "📰 NEWS"}
                          </span>
                          <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{task.name}</span>
                        </div>
                        <button className="btn btn-ai" style={{ fontSize: 9, padding: "3px 10px" }}
                          onClick={async () => {
                            try {
                              const res = await fetch("/api/classify", {
                                method: "POST", headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                                  ...
                                  taskJobTitleKeywords: task.jobTitleKeywords,
                                  taskSources: task.sources,
                                }),
                              });
                              if (res.ok) {
                                const data = await res.json();
                                if (data.scoringPrompt) {
                                  setTaskDefs(prev => prev.map(t => t.id === task.id ? { ...t, scoringPrompt: data.scoringPrompt } : t));
                                }
                              }
                            } catch (e) { console.error(e); }
                          }}>
                          <Icons.Sparkle /> Regenerate
                        </button>
                      </div>
                      <div style={{ fontSize: 10, color: "var(--text-tertiary)", marginBottom: 6 }}>{task.description?.slice(0, 120)}{task.description?.length > 120 ? "..." : ""}</div>
                      <textarea className="input textarea"
                        value={task.scoringPrompt || ""}
                        onChange={e => {
                          const val = e.target.value;
                          setTaskDefs(prev => prev.map(t => t.id === task.id ? { ...t, scoringPrompt: val } : t));
                        }}
                        placeholder="No scoring prompt — will use default during scan. Click 'Regenerate' to create one."
                        style={{ minHeight: 70, fontSize: 11, lineHeight: 1.5, width: "100%", background: "var(--bg-main)", border: task.scoringPrompt ? "1px solid rgba(191,163,90,0.3)" : "1px solid var(--border)" }} />
                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                        <span style={{ fontSize: 9, color: "var(--text-tertiary)" }}>
                          {task.scoringPrompt ? `${task.scoringPrompt.length} chars` : "⚠️ Empty — default scoring will be used"}
                        </span>
                        <span style={{ fontSize: 9, color: "var(--text-tertiary)" }}>
                          Keywords: {[...(task.keywords || []), ...(task.jobTitleKeywords || [])].slice(0, 5).join(", ")}{[...(task.keywords || []), ...(task.jobTitleKeywords || [])].length > 5 ? "..." : ""}
                        </span>
                      </div>
                    </div>
                  ))}

                  {taskDefs.filter(t => !t.scoringPrompt).length > 0 && (
                    <div style={{ marginTop: 12, padding: 12, border: "1px dashed rgba(191,163,90,0.3)", borderRadius: 8, textAlign: "center" }}>
                      <div style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 8 }}>
                        {taskDefs.filter(t => !t.scoringPrompt).length} tasks have no scoring prompt
                      </div>
                      <button className="btn btn-ai" onClick={async () => {
                        const empty = taskDefs.filter(t => !t.scoringPrompt);
                        for (const task of empty) {
                          try {
                            const res = await fetch("/api/classify", {
                              method: "POST", headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
                        ...
                        <Icons.Sparkle /> Auto-Generate All Missing Prompts
                      </button>
                    </div>
                  )}
                </div>
