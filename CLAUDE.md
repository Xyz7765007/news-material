...
- `POST /api/sidekick/comment` — **Posts a LinkedIn comment via Unipile** (Kunal Jul-20 auto-comment; reverses the old copy-only rule — sanctioned, approval = the explicit operator click/command). Body `{ baseId, postUrl, text, accountId?, dryRun? }`. Extracts `urn:li:activity:*` social_id from `postUrl`, resolves the account to the campaign's `LinkedIn Account ID` (Veloka = Kunal's own `iqFY_VqkTty6Ns6q-kpxvA`; env `VELOKA_UNIPILE_ACCOUNT_ID` or hardcoded fallback), strips em dashes + caps 1250, then `POST /posts/{social_id}/comments`. `dryRun:true` validates + returns `wouldPost:{social_id, account:{id,name}, text}` WITHOUT posting (the account NAME lets the caller confirm identity before any live post). The sidekick apps reach it through their `/api/comment-linkedin` proxy.
