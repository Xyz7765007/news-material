...
//   2. Top X ICP match — counted as a strong signal (per Kunal, May 16:
...
export function scoreLeadFromTasks(lead, tasks = []) {
  ...
  for (const t of tasks) {
    ...
    if (type === "top_x" && age <= TOP_X_RECENCY_DAYS) {
      ...
      const summary = String(scoreReason).split(/[.\n]/)[0].slice(0, 100).trim();
...
// Two Lead records can share the same LinkedIn URL (Apollo + Crunchbase
// imports often overlap). Keep the highest-scored one. Filling continues
// until we have maxResults unique candidates.
function dedupAndSlice(scored, maxResults) {
  const seen = new Set();
  const unique = [];
  for (const item of scored) {
    const key = normalizeUrl(item.linkedinUrl);
