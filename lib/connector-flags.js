...
//
// DEFAULTS (per family) — CORRECTED 2026-08-18. Both now default ON:
//   LINKEDIN_CONNECTORS_ENABLED → defaults ON (re-enabled by Samarth 2026-06-15;
//     disable with the literal "false"). Follows the ROLE_GATE_ENABLED convention.
//   LINKEDIN_DMS_ENABLED        → defaults ON since 2026-07-07 (Kunal asked for
//     Veloka outreach to go live). Disable with the literal "false".
//
// ⚠ THIS BLOCK USED TO SAY DMs DEFAULT **OFF**, "enabled ONLY when the env var
// is exactly true — any other value, including unset, keeps it OFF". That
// stopped being true on 2026-07-07 and the header was never updated, so the
// file's own summary contradicted the export 30 lines below it. This is the
// flag that gates REAL LinkedIn sends (connection requests and DMs), so the
// stale reading was the dangerous direction to be wrong in: anyone checking
// "can this send without me setting anything?" got the answer "no" when the
// true answer is "yes". The only thing currently stopping automatic sends is
// that the 4-hourly outreach cron schedule is commented out — NOT this flag.
//
...
// 2026-07-07: re-enabled by default for Veloka outreach from Kunal's account
// (Kunal asked for the outreach to go live). Mirrors the CONNECTORS convention —
// disable again with LINKEDIN_DMS_ENABLED=false. NOTE: the shared 4-hourly
// outreach cron is paused (outreach-cron.yml schedule commented) so nothing
// auto-sends; sends are driven manually per campaign until we re-schedule.
export const LINKEDIN_DMS_ENABLED =
  String(process.env.LINKEDIN_DMS_ENABLED ?? "true").trim().toLowerCase() !== "false";
