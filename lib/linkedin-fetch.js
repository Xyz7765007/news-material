...
export async function fetchLinkedInProfile(linkedinUrl) {
  ...
  if (resp.status === 429) {
    ...
    // Distinguish monthly-quota-exhausted from temporary rate limit.
    // Monthly quota = fatal for the entire scan (no point firing more calls).
    // Per-minute rate limit = transient (we already backed off 3 times).
    const isMonthlyQuota = /MONTHLY quota/i.test(body) || /exceeded the MONTHLY/i.test(body);
    return {
      ok: false,
      error: isMonthlyQuota ? "monthly_quota_exhausted" : "http_429",
      statusCode: 429,
      fatal: isMonthlyQuota, // signals scan handler to abort the entire batch
