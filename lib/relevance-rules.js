...
// Airtable formula string-literal escaping. TWO characters can break out of a
// "double-quoted" literal, not one:
//
//   "  — ends the literal outright.
//   \  — is the ESCAPE character, so a value ending in a backslash escapes the
//        closing quote and swallows the rest of the formula. `FIND("acme\",
//        LOWER({Company} & ""))` is not a formula, it is a parse error.
//
// Airtable does NOT support "" doubling inside filterByFormula, so the safe
// move is to strip both characters entirely. Rule Values are job titles,
// company names and task-type strings; neither character is ever meaningful in
// one, so nothing legitimate is lost.
//
// WHY THIS IS WORTH GUARDING AGAINST A "NOBODY WOULD TYPE THAT" INPUT. These
// values are operator free-text, and the string they land in is the READ filter
// for BOTH /api/sidekick/feed and /api/sidekick/count. A malformed formula does
// not degrade the offending rule — Airtable 422s the whole request, and neither
// route recognises a parse error (their 422 handlers look for UNKNOWN_FIELD_NAME
// specifically), so one stray backslash takes that base's entire card feed down
// and reports it as a missing field. One character of input, total feed outage.
