...
// ESM export. This was `module.exports = {...}` — the ONE CommonJS file among
// the seventeen in lib/, which was invisible while Next's bundler tolerated
// both. It stopped being tolerable when lib/package.json declared the folder
// ESM (so the test suite could import these modules at all): under that marker
// a .js file IS a module, `module` is not defined, and the named imports in
// app/api/scan/route.js resolved to undefined — a build warning, and a news
// scan that would have thrown at runtime on the first URL it tried to decode.
//
// The import site is deliberately UNCHANGED (`import { decodeGoogleNewsUrl,
// drainFailureStats } from "@/lib/google-news-decoder"`); only this line moved,
// so the live /api/scan path sees exactly the same two functions it always did.
export { decodeGoogleNewsUrl, extractEncodedPart, drainFailureStats };
