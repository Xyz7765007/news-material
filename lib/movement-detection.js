...
export function classifyMovement({
  ...
  const previous = profile.experiences[1] || null;
  ...
  // Step 1: Match current company to stored account
  const matchesStoredAccount = matchCompanyNames(storedAccount, current.company);
  ...
  // ────────────────────────────────────────────────────────────
  // CASE A: Current company matches stored account
  // ────────────────────────────────────────────────────────────
  if (matchesStoredAccount.matched) {
    if (!withinWindow) {
    ...
    return {
      ...
      details: { ...baseDetails, confidence: matchesStoredAccount.confidence },
