...
// Classifies movement based on three categories (per Samarth's spec):
//
...
// Per operator feedback (2026-06): some roles are NOT a relevant buying
// signal for Veloka regardless of how strong the movement is. A Product
// Marketing Manager who gets promoted/hired still isn't who we sell to,
// so they should not surface at score 90 in the feed.
//
// Movement scoring is otherwise hardcoded (Hired/Promoted 90, Exited 75)
// and has no role awareness — this list adds it. Matching is substring,
// case-insensitive, against the lead's current/stored title.
//
// To tune later: add/remove patterns here (lowercase). Keep it tight —
// only roles the operator has explicitly said are not relevant.
const IRRELEVANT_ROLE_PATTERNS = [
  "product marketing",   // pt5: "Product Marketing is not a relevant role for us"
];

// Returns true if the given title matches an irrelevant role.
...
export function buildTaskFromMovement(classification, lead) {
  ...
  // Role relevance (pt5): some roles are not a relevant buying signal for
  // Veloka no matter how strong the movement. Check the lead's relevant
  // title — current title for Hired/Promoted (where they are now), stored
  // title for Exited (the role they held at our account). If it matches an
  // irrelevant role, floor the score to 15 so it drops out of the feed
  // (the feed sorts by Score desc and the operator works top-down).
