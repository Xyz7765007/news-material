    ...
    for (let i = 0; i < slugs.length; i += BATCH_SIZE) {
      ...
      while (currentTokenIndex < tokens.length && !batchDone) {
        ...
        try {
          ...
          const isExhausted = res.status === 402 || res.status === 403 || res.status === 429;
          if (isExhausted || (!res.ok && res.status !== 502 && res.status !== 503)) {
            const err = await res.text();
            const errLower = err.toLowerCase();
            if (isExhausted || errLower.includes("maximum usage") || errLower.includes("billing") || errLower.includes("exceeded") || errLower.includes("platform-feature-disabled") || errLower.includes("hard limit")) {
