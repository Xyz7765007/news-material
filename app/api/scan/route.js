...
function isCreditExhausted(status, body) {
  ...
  return text.includes("maximum usage") || text.includes("billing") || text.includes("exceeded") || text.includes("insufficient");
}
 ...
 */
async function apifyCallWithFallback(actorId, input, timeoutMs = 25000) {
...
async function scanJobsBatch(companies, taskDefs) {
  ...
  const input = { urls: allUrls, maxItems: 25 * companies.length };

  // Use fallback helper — tries all 3 tokens automatically
  const { data, error } = await apifyCallWithFallback(actorId, input, 25000);
